...
var CLIENTS_HEADERS = [
  ...
  'creditsOwed'   // server-managed running credit balance (session-quota engine)
];
...
// Sheet-managed rates: therapists are added/updated by editing the TherapistRates
// sheet (no code redeploy). Auto-seeded once from the hardcoded maps below.
// Cached 120s via CacheService so per-session calls don't re-read the sheet;
// a sheet edit therefore takes effect within ~2 minutes.
...
// (the old full-sheet _writeAll here was the save-path hotspot). Locates the row
// by scanning the id column (like the SessionLog upsert). Fail-soft: no-hit
// (client deleted mid-flight) changes nothing.
  ...
  try {
    ...
    if (matchedClient) {
      ...
      if (balance !== origCredits) {
        ...
        _writeCreditsOwed(clientsSh, clientId, balance);   // single cell, not whole sheet
      }
